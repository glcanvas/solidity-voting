...
type Favorites = { electionName: string, candidates: SignerWithAddress[], score: number }
...
async function voteForCandidate(contract: Voting, election: string, from: SignerWithAddress, to: SignerWithAddress) {
  contract = contract.connect(from);
  await expect(contract.voteForCandidate(election, to.address, { value: ENOUGH_PAYABLE_ETHER }))
...
async function checkElectionInfo(contract: Voting, info: ElectionInfo) {
  ...
  expect(electionResponse[5]).to.be.eql(info.candidates.map(x => x.candidate.address));
  expect(electionResponse[6].map(x => x.toNumber())).to.be.eql(info.candidates.map(x => x.count));
}
...
async function checkFavorites(contract: Voting, favorites: Favorites) {
  ...
  expect(favoritesResponse[0]).to.be.eql(favorites.candidates.map(x => x.address));
...
const candidatesOrder = [[0, 1, 1], [1, 0, 1], [1, 1, 0]];
const winners = [1, 2, 3, 10, 100, 777].map(x => BigNumber.from(x));
const worths = ["0.01", "1", "0.3", "0.7", "99"].map(x => ethers.utils.parseEther(x));

async function checkEmptyElection(voting: Voting) {

  await checkElectionInfo(voting, createEmptyElection("election", true));
  await checkFavorites(voting, { electionName: "election", score: 0, candidates: [] });
...
async function checkContractInfoWithBalance(voting: Voting, isActive: boolean, author: SignerWithAddress, nonAuthor: SignerWithAddress) {
  expect(await ethers.provider.getBalance(voting.address))
    .to.be.equal(ENOUGH_PAYABLE_ETHER);
  ...
  await checkFavorites(voting, { electionName: "election 1", candidates: [nonAuthor], score: 1 });
...
describe("Voting", function() {
  ...
  it("change owner", async function() {
    ...
    await expect(voting.transferOwnership(nonAuthor.address)).to.be.revertedWith("Changing the owner is forbidden");
    voting = voting.connect(nonAuthor);
    await expect(voting.transferOwnership(nonAuthor.address)).to.be.revertedWith("Ownable: caller is not the owner");
  });
  ...
  it("duplicate election", async function() {
    const voting = await deployContract();
    await voting.createElection("election");
    await expect(voting.createElection("election")).to.be.revertedWith("Election already exists");
  });

  it("election not found", async function() {
    const voting = await deployContract();
    await expect(voting.getFavorites("election")).to.be.revertedWith("Election not found");
  });
  ...
  it("create election by non owner", async function() {
    ...
    const [author, nonAuthor, ...other] = await ethers.getSigners();
    voting = voting.connect(nonAuthor);
    await expect(voting.createElection("election")).to.be.revertedWith("Ownable: caller is not the owner");
  ...
  it("create two elections", async function() {
    ...
    const internalCheck = async (active: boolean) => {
      ...
      await checkElectionInfo(voting, createEmptyElection("election 1", active));
      await checkElectionInfo(voting, createEmptyElection("election 2", active));
    };
  ...
  it("create election and vote for candidate", async function() {
    ...
    await expect(voting.voteForCandidate("election 1", nonAuthor.address, { value: ENOUGH_PAYABLE_ETHER }))
  ...
  it("vote with wrong balance", async function() {
    ...
    await expect(voting.voteForCandidate("election 1", nonAuthor.address, { value: MORE_ENOUGH_PAYABLE_ETHER }))
      .to.be.revertedWith("Wrong ethers amount, must be 0.01");

    await expect(voting.voteForCandidate("election 1", nonAuthor.address, { value: NOT_ENOUGH_PAYABLE_ETHER }))
      .to.be.revertedWith("Wrong ethers amount, must be 0.01");

    expect(await ethers.provider.getBalance(voting.address))
      .to.be.equal(ZERO_ETHER);

    await checkElectionInfo(voting, createEmptyElection("election 1", true));
    await checkFavorites(voting, { electionName: "election 1", candidates: [], score: 0 });
  ...
  it("vote twice", async function() {
    ...
    await expect(voting.voteForCandidate("election 1", author.address, { value: ENOUGH_PAYABLE_ETHER }))
  ...
  it("vote and get favorite distribution 'nonAuthor1, 2'", async function() {
    ...
    await checkFavorites(voting, { electionName: "election 1", candidates: [nonAuthor1], score: 2 });
    expect(await voting.getElections()).to.be.eql(["election 1"]);
  });
  ...
  it("vote and get favorite distribution 'author, 1', 'nonAuthor1, 1'", async function() {
    ...
    await checkElectionInfo(voting, {
      ...
      isActive: true,
      rewardDistributed: false,
      candidates: [{ candidate: author, count: 1 }, { candidate: nonAuthor1, count: 1 }],
      electorate: [author, nonAuthor1]
    });
    await checkFavorites(voting, { electionName: "election 1", candidates: [author, nonAuthor1], score: 1 });
    expect(await voting.getElections()).to.be.eql(["election 1"]);
  });

  it("distribute reward error", async function() {
    let voting = await deployContract();
    await expect(voting.distributeReward(BigNumber.from(0), BigNumber.from(10)))
      .to.be.revertedWith("At least one winner must be exist");
    await expect(voting.distributeReward(BigNumber.from(10), BigNumber.from(0)))
  ...
  it("finish election reverts", async function() {
    let voting = await deployContract();
    await voting.createElection("election");
    await expect(voting.finishElection("election")).to.be.revertedWith("Election is active");
    await expect(voting.finishElection("some name")).to.be.revertedWith("Election not found");
  });
  ...
  it("finish election one winner", async function() {
    ...
    await shiftNextTime(4);
    let [perAddressReward, taxes] = await voting.distributeReward(BigNumber.from(1), ENOUGH_PAYABLE_ETHER);
    await expect(voting.finishElection("election"))
      .to.emit(voting, "DistributeReward")
  ...
  it("finish election two winners", async function() {
    ...
    await shiftNextTime(4);
    let [perAddressReward, taxes] = await voting.distributeReward(BigNumber.from(2), ENOUGH_PAYABLE_ETHER.mul(BigNumber.from(2)));
    await expect(voting.finishElection("election"))
      .to.emit(voting, "DistributeReward")
    ...
    await checkElectionInfo(voting, {
      ...
      isActive: false,
      rewardDistributed: true,
      candidates: [{candidate: wallets[0], count: 1}, {candidate: wallets[1], count: 1}],
  ...
  it("withdraw taxes", async function() {
    ...
    await expect(voting.getTaxes()).to.be.revertedWith("Ownable: caller is not the owner");
  });

  for (let e of electorateOrder) {
    for (let c of candidatesOrder) {
      it("vote, distribution: 'author, 1', 'nonAuthor 2' " +
        "electorate:" + e + " candidates:" + c,
        async function() {
          let voting = await deployContract();
          await voting.createElection("election 1");
          const wallets = await ethers.getSigners();

          await voteForCandidate(voting, "election 1", wallets[e[0]], wallets[c[0]]);
          await voteForCandidate(voting, "election 1", wallets[e[1]], wallets[c[1]]);
          await voteForCandidate(voting, "election 1", wallets[e[2]], wallets[c[2]]);

          let candidateOrder = wallets[c[0]] == wallets[0]
            ? [{ candidate: wallets[0], count: 1 }, { candidate: wallets[1], count: 2 }]
            : [{ candidate: wallets[1], count: 2 }, { candidate: wallets[0], count: 1 }];
          await checkElectionInfo(voting, {
            electionName: "election 1",
            isActive: true,
            rewardDistributed: false,
          ...
          await checkFavorites(voting, { electionName: "election 1", candidates: [wallets[1]], score: 2 });
        });
    }
  }
  ...
  for (let winner of winners) {
    for (let worth of worths) {
      it("distribute reward winner:" + winner + " worth:" + worth, async function() {
        let voting = await deployContract();
        let [perAddressReward, taxes] = await voting.distributeReward(winner, worth);
